<script lang="ts">
	...
	import EditMessageDropdown from './edit-message-dropdown.svelte';
...
<div class="group relative ml-8 w-fit max-w-2xl rounded-lg bg-primary px-3 py-2">
	...
	{:else}
		<div class="flex flex-row items-center gap-1.5 text-gray-300">
			<Ban class="size-4" />
			<p>You deleted this message</p>
		</div>
	{/if}

	<div class="flex flex-row items-center justify-end text-xs font-medium text-gray-300">
		{#if message.editedAt}
	...
	<!-- Dropdown -->
	<EditMessageDropdown class="absolute -right-0.5 -top-0.5" bind:isDropdownOpen bind:isEditOpen />
